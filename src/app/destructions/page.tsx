...
export default function DestructionsPage() {
  const { user } = useAuth();
  const [Destructions, setDons] = useState<Destruction[]>([]);
  ...
  const fetchDons = async () => {
    ...
    const { data, error } = await supabase
      .from("destructions")
      .select(`*, creator:app_users!destructions_created_by_fkey(full_name)`)
  ...
  const handleViewDetails = async (Destruction: any) => {
    const { data } = await supabase.from("destruction_lines").select("*, articles(designation)").eq("destruction_id", Destruction.id).order('id');
    setSelectedDon({ ...Destruction, lines: data || [] });
    setIsDetailsModalOpen(true);
  };

  const handleEditDon = async (Destruction: any) => {
    const { data } = await supabase.from("destruction_lines").select("*, articles(designation)").eq("destruction_id", Destruction.id).order('id');
    ...
    setEditingDonId(Destruction.id);
    setBeneficiaryName(Destruction.reason || "");
    setIssueDate(Destruction.date);
    setObservations(Destruction.observations || "");
    setLines(formattedData);
    loadFormData();
    setIsModalOpen(true);
  };

  const handleDeleteDon = (Destruction: any) => {
    showConfirm(`Voulez-vous vraiment supprimer le Destruction ${Destruction.destruction_number} ?`, async () => {
      try {
        if (Destruction.status === "VALIDATED") {
          await supabase.from("stock_movements").delete().eq("reference_id", Destruction.id);
        }
        const { error } = await supabase.from("destructions").delete().eq("id", Destruction.id);
        if (error) throw error;
        fetchDons();
        showNotification("Destruction supprimé.", "success");
      } catch (error: any) {
  ...
  const handlePrintDon = async (donToPrint?: any) => {
    let Destruction = donToPrint;
    if (!donToPrint || donToPrint.nativeEvent) {
      Destruction = selectedDon;
    }
    if (!Destruction) return;

    let linesToPrint = Destruction.lines;
    if (!linesToPrint) {
      const { data } = await supabase.from("destruction_lines").select("*, articles(designation)").eq("destruction_id", Destruction.id).order('id');
      linesToPrint = data || [];
    }
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .Destruction-title h2 {
              margin: 0;
              font-size: 16px;
              text-transform: uppercase;
            }
            .Destruction-title h3 {
              margin: 2px 0 0 0;
              font-size: 14px;
            }
        ...
        <body>
          <div class="header">
            ...
            <div class="client-info-container">
              <div class="date-page">
                <span>Abidjan, le ${new Date(Destruction.date).toLocaleDateString('fr-FR')}</span>
                <span>Page 1/1</span>
              </div>
              <div class="client-box">
                <p style="font-weight: bold; margin-bottom: 5px;">BÉNÉFICIAIRE :</p>
                <p style="font-weight: bold; font-size: 12px;">${Destruction.reason || 'Non renseigné'}</p>
              </div>
            </div>
          </div>

          <div class="Destruction-title">
            <h2>BON DE Destruction</h2>
            <h3>N° ${Destruction.destruction_number}</h3>
          </div>
          ...
          <div class="footer">
            <p class="conditions">Document établi pour valoir ce que de droit concernant les Destructions de l'entreprise.</p>
          </div>
        </body>
      </html>
    `;
  ...
  const removeLine = (index: number) => {
    showConfirm("Voulez-vous vraiment retirer cet article du Destruction ?", () => {
  ...
  const handleSave = async (status: "DRAFT" | "VALIDATED") => {
    ...
    try {
      ...
      if (editingDonId) {
        ...
        const { error: donError } = await supabase.from("destructions").update({
          reason: beneficiaryName,
          date: issueDate,
        ...
        await supabase.from("destruction_lines").delete().eq("destruction_id", editingDonId);
      } else {
        const formatStr = settings?.destruction_format || "DES-YYYYMM-XXXX";
        ...
        const { data: lastDon } = await supabase
          .from('destructions')
          .select('destruction_number')
          .ilike('destruction_number', `${prefix}%`)
          .order('destruction_number', { ascending: false })
          .limit(1);
          
        let seqNum = 1;
        if (lastDon && lastDon.length > 0) {
           const lastSeq = parseInt(lastDon[0].destruction_number.split('-').pop() || "0", 10);
           if (!isNaN(lastSeq)) {
             seqNum = lastSeq + 1;
           }
        }
        ...
        const { data: donData, error: donError } = await supabase.from("destructions").insert([{
          destruction_number: donNum,
          reason: beneficiaryName,
          date: issueDate,
          observations: observations,
          status: status,
      ...
      const { error: linesError } = await supabase.from("destruction_lines").insert(linesToInsert);
      if (linesError) throw linesError;

      if (status === "VALIDATED") {
        const movementsToInsert = lines.map(l => ({
          article_code: l.article_code,
          type: "OUT_DESTRUCTION",
          quantity: l.quantity,
      ...
      showNotification("Destruction sauvegardé avec succès !", "success");
  ...
  const filtered = Destructions.filter(d => 
    d.destruction_number.toLowerCase().includes(search.toLowerCase()) || 
  ...
  return (
    <div className="space-y-6 relative">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Gestion des Destructions</h2>
        ...
        >
          <Plus className="h-4 w-4 mr-2" /> Nouveau Destruction
        </button>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider">
                <th className="px-6 py-4 font-medium">N° Destruction</th>
              ...
              {isLoading ? (
                ...
                filtered.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition">
                    <td className="px-6 py-4 font-semibold text-slate-700">{item.destruction_number}</td>
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-gray-500">Aucun Destruction trouvé.</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h3 className="text-lg font-bold text-gray-800">
                {editingDonId ? "Modifier le Destruction" : "Nouveau Destruction"}
              </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={4}
                    placeholder="Détails, motif du Destruction..."
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex justify-between items-center bg-gray-50/50">
              <div className="text-sm text-gray-500">
                L'enregistrement d'un Destruction validé décrémente automatiquement le stock.
              </div>
              <div className="flex gap-3">
                ...
                >
                  <Save className="h-4 w-4 mr-2" /> Valider le Destruction
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <h3 className="text-lg font-bold text-gray-800">Détails du Destruction - {selectedDon.destruction_number}</h3>
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <p className="font-bold text-gray-800">{selectedDon.reason}</p>
                </div>
                <div>
